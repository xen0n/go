...
func rewriteRegRegMOV(newprog obj.ProgAlloc, p *obj.Prog) {
	...
	// Use SLLID/SRAID to extend.
	case AMOVB:
		rewriteMOVWithShifts(newprog, p, ASRAID, 8)
	case AMOVH:
		rewriteMOVWithShifts(newprog, p, ASRAID, 16)

	// Use SLLID/SRLID to extend.
	case AMOVHU:
		rewriteMOVWithShifts(newprog, p, ASRLID, 16)
	case AMOVWU:
		rewriteMOVWithShifts(newprog, p, ASRLID, 32)
	}
}

// rewriteMOVWithShifts rewrites a register-to-register MOV with a pair of shifts.
//
// MOVx From, To -> SLLID $shamt, From, To & <SHR> $shamt, To, To
func rewriteMOVWithShifts(
	...
	toReg := p.To
	shamt := int64(64 - bitwidth)

	// MOVx From, To -> SLLID $shamt, From, To
	p.As = ASLLID
	p.SetRestArgs([]obj.Addr{p.From})
	p.From = obj.Addr{Type: obj.TYPE_CONST, Offset: shamt}

	// append specified shift-right op
	// <SHR> $shamt, To, To
	p = obj.Appendp(p, newprog)
	p.As = shr
	p.From = obj.Addr{Type: obj.TYPE_CONST, Offset: shamt}
	p.SetRestArgs([]obj.Addr{toReg})
	p.To = toReg
}
